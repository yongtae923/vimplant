...
outputfolder = f"C:/Users/user/YongtaeC/vimplant0812/data/0920_new_targets_{subj_id}/"
...
if ImageTargetGenerator is not None:
    image_generator = ImageTargetGenerator()
    ...
    for letter in letters:
        ...
        if target_map is None:
            print(f"타겟 맵 생성 중: {letter}")
            target_map = image_generator.generate_from_text(letter, target_name, 
                                                           size=WINDOWSIZE, font_size=700, fwhm=50)
        
        if target_map is not None:
            image_targets.append(target_map)
            image_target_names.append(f"targ-{letter.lower()}")
    
    # 겹친 알파벳 타겟 맵 생성 (A~C, A~Z)
    overlapping_targets = [
        ("ABC", "letters_abc"),
        ("ABCDEFGHIJKLMNOPQRSTUVWXYZ", "letters_az")
    ]
    
    for letters_str, target_name in overlapping_targets:
        print(f"겹친 알파벳 타겟 맵 생성 중: {letters_str}")
        target_map = image_generator.generate_overlapping_alphabet_target(
            letters_str, target_name, size=WINDOWSIZE, font_size=700, fwhm=50
        )
        if target_map is not None:
            image_targets.append(target_map)
