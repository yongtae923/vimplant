...
def create_grid_v2(
    desired_center,
    shank_length=12,
    n_contactpoints_shank=None,         # 백워드 호환(이전: z축 개수)
    spacing_along_xy=1,                 # 백워드 호환(이전: x,y 동일 간격)
    offset_from_origin=0,
    *,
    n_x=None, n_y=None, n_z=None,       # 새: x,y,z 개수
    spacing_x=None, spacing_y=None      # 새: x,y 간격(mm)
):
    """
    n_x, n_y, n_z : 각 축의 컨택트 개수(정수)
    spacing_x, spacing_y : x,y축 간격(mm). z축 간격은 shank_length와 n_z로부터 자동결정
    """
    # --- 하위호환 처리 ---
    if n_z is None:
        # 이전 코드와 동일한 의미: "샹크(z축)상의 컨택트 개수"
        n_z = n_contactpoints_shank if n_contactpoints_shank is not None else 10
    if n_x is None: n_x = n_contactpoints_shank if n_contactpoints_shank is not None else n_z
    if n_y is None: n_y = n_contactpoints_shank if n_contactpoints_shank is not None else n_z

    if spacing_x is None: spacing_x = spacing_along_xy
    if spacing_y is None: spacing_y = spacing_along_xy

    # --- 내부 파라미터 구성 ---
    contacts_perShank   = int(n_z)     # z축 컨택트 수
    num_shanks_perComb  = int(n_x)     # x축 (한 comb 안의 샹크 개수)
    number_of_combs     = int(n_y)     # y축 (comb 개수)

    # z축 위치(컨택트) 배치: shank_length를 n_z 구간으로 등분
    contacts_position_lenght_singleShank = np.linspace(offset_from_origin, shank_length, num=contacts_perShank)

    # 한 comb에서 각 shank마다 동일한 z프로파일을 복제
    contacts_position_lenght = np.tile(
        contacts_position_lenght_singleShank, (num_shanks_perComb, 1)
    )

    # 각 shank의 (x=0, y=0, z=contacts) 좌표 구성
    shankList = []
    for sh in range(num_shanks_perComb):
        coords_shank = np.zeros((contacts_perShank, 3))
        for contact in range(contacts_perShank):
    ...
    # x축 방향으로 shank 간격 적용
    for i in range(num_shanks_perComb):
        shankList[i][:, 0] += i * spacing_x

    # (필요 시) 전극 절대각 보정 로직은 기존 그대로 두되, spacing_x/y만 반영
    BRAIN_ANGLE = 0
    comb_horizontal_angle = radians(BRAIN_ANGLE)
    tan = math.tan(comb_horizontal_angle)
    ...
    shankOriginList = []
    for i in range(num_shanks_perComb):
        hor_distance_origin = i * spacing_x
        vert_distance_origin = hor_distance_origin * tan
        shankOrigin = np.array([i * spacing_x, 0, vert_distance_origin])
    ...
    for sh in range(num_shanks_perComb):
        ...
        for contact in range(contacts_perShank):
            ...
            z_dist_to_origin = np.abs(z_contact - z_origin)
            x_angle = z_dist_to_origin * tan
            shankList[sh][contact][0] = x_contact + x_angle

    # comb 하나를 (n_x * n_z)개 포인트로 만들고
    comb = np.asarray(shankList).reshape(num_shanks_perComb * contacts_perShank, 3)
    aux_ones = np.ones((comb.shape[0], 1)).astype('float32')
    comb = np.hstack((comb, aux_ones)).T
    ...
    # 원하는 중심으로 한 comb 이동
    rotation_angles = (0, 0, 0)
    x_new_comb, y_new_comb, z_new_comb = translate_cube(
        comb_center, desired_center, rotation_angles, comb
    )
    comb = cube_from_points(x_new_comb, y_new_comb, z_new_comb)

    allContactPointsList = [comb]
    contacts_xyz = np.asarray(comb)[0:3, :]

    # y축 방향으로 comb 복제(number_of_combs-1개), 간격 spacing_y
    HEIGHT_DIFFERENCE = 0
    for _ in range(number_of_combs - 1):
        x_new, y_new, z_new = translate_cube(
            comb_center,
            comb_center[0:3] + [0, spacing_y, HEIGHT_DIFFERENCE],
            rotation_angles,
            comb
        )
        comb = cube_from_points(x_new, y_new, z_new)
        allContactPointsList.append(comb)
        contacts_xyz = np.hstack((contacts_xyz, np.asarray(comb)[0:3, :]))
