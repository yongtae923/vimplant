...
def visualize(surf_path: str,
              ...
              offset_mm: float,
              shank_len_mm: float,
              second_alpha: Optional[float] = None,
              second_beta: Optional[float] = None,
              second_offset_mm: Optional[float] = None,
              second_shank_len_mm: Optional[float] = None,
              show_b_main: bool = False,
              show_b_channels: bool = False,
    ...
    lines = [f"A: α={alpha:.2f}°, β={beta:.2f}°, off={offset_mm:.2f} mm, L={shank_len_mm:.2f} mm"]
    if second_alpha is not None and second_beta is not None and second_offset_mm is not None and second_shank_len_mm is not None:
        lines.append(f"B: α={second_alpha:.2f}°, β={second_beta:.2f}°, off={second_offset_mm:.2f} mm, L={second_shank_len_mm:.2f} mm")
    meta_parts = []
    if sample_id:
    ...
    # second electrode (if provided) — use distinct colors (orange/blue/magenta)
    if second_alpha is not None and second_beta is not None and second_offset_mm is not None and second_shank_len_mm is not None:
        beta2_for_vis = -float(second_beta)
        d2 = rotation_yaw_pitch(second_alpha, beta2_for_vis)
        d2 = Rg @ d2
        p1b = p0 + float(second_offset_mm) * d2
        p2b = p1b + float(second_shank_len_mm) * d2
        # main B (hidden by default; controlled via show_b_main)
        if show_b_main:
            plotter.add_mesh(pv.Line(p0, p2b), color="#ff8800", line_width=5)
            plotter.add_mesh(pv.Sphere(radius=max(float(second_shank_len_mm), 10.0) * 0.02, center=p1b), color="magenta", opacity=0.9)
            if shank_radius_mm and shank_radius_mm > 0.0:
                shank_line_b = pv.Line(p1b, p2b)
                shank_tube_b = shank_line_b.tube(radius=shank_radius_mm)
                plotter.add_mesh(shank_tube_b, color="#666666", opacity=float(np.clip(shank_opacity, 0.05, 1.0)))
        # B channels (blue points) controlled via show_b_channels
        if show_b_channels and num_channels and num_channels > 0 and contact_spacing_mm > 0:
            channel_positions_b = [p1b + i * contact_spacing_mm * d2 for i in range(num_channels)]
            chan_cloud_b = np.vstack(channel_positions_b)
            plotter.add_mesh(pv.PolyData(chan_cloud_b), color="blue", render_points_as_spheres=True, point_size=10)
    ...
    # auxiliary for second electrode (alpha+90) if provided
    if show_alpha90 and (second_alpha is not None and second_beta is not None and second_offset_mm is not None and second_shank_len_mm is not None):
        d_90_b = rotation_yaw_pitch(float(second_alpha) + 90.0, -float(second_beta))
        d_90_b = Rg @ d_90_b
        p1_90_b = p0 + float(second_offset_mm) * d_90_b
        p2_90_b = p1_90_b + float(second_shank_len_mm) * d_90_b
        # different palette to distinguish from A
        plotter.add_mesh(pv.Line(p0, p2_90_b), color="#00ccff", line_width=3)
        plotter.add_mesh(pv.Sphere(radius=max(float(second_shank_len_mm), 10.0) * 0.02, center=p1_90_b), color="#ff66cc", opacity=0.8)
        if num_channels and num_channels > 0:
            channel_positions_90_b = [p1_90_b + i * 1.0 * d_90_b for i in range(num_channels)]
            chan_cloud_90_b = np.vstack(channel_positions_90_b)
            plotter.add_mesh(pv.PolyData(chan_cloud_90_b), color="dodgerblue", render_points_as_spheres=True, point_size=channel_point_size)

        if show_grid_channels and grid_rows >= 1 and grid_cols >= 1 and grid_row_spacing_mm > 0.0 and grid_col_spacing_mm > 0.0:
            refb = np.array([0.0, 0.0, 1.0])
            if abs(np.dot(refb / np.linalg.norm(refb), d_90_b)) > 0.99:
                refb = np.array([1.0, 0.0, 0.0])
            ub = np.cross(refb, d_90_b)
            nub = np.linalg.norm(ub)
            if nub == 0.0:
                ub = np.array([1.0, 0.0, 0.0])
                nub = 1.0
            ub = ub / nub
            vb = np.cross(d_90_b, ub)
            ...
            for r in range(int(grid_rows)):
                for c in range(int(grid_cols)):
                    ptb = p1_90_b + (r * grid_row_spacing_mm) * ub + (c * grid_col_spacing_mm) * vb
                    grid_points_b.append(ptb)
                    labels_b.append(f"{r},{c}")
            grid_points_b = np.vstack(grid_points_b) if grid_points_b else np.empty((0, 3))
            if grid_points_b.shape[0] > 0:
                if show_channel_labels:
                    plotter.add_point_labels(grid_points_b, labels_b, point_size=18, text_color="black", font_size=12)

                # optional save with suffix _B to avoid overwrite
                if save_channels_csv or save_channels_npy:
                    idx = 0
                    rows_b = []
                    for r in range(int(grid_rows)):
                        for c in range(int(grid_cols)):
                            rows_b.append([idx, r, c, grid_points_b[idx, 0], grid_points_b[idx, 1], grid_points_b[idx, 2]])
                            idx += 1
                    arr_b = np.array(rows_b, dtype=float)
                    if save_channels_npy:
                        base, ext = os.path.splitext(save_channels_npy)
                        out_npy_b = f"{base}_B{ext}"
                        os.makedirs(os.path.dirname(out_npy_b), exist_ok=True)
                        np.save(out_npy_b, arr_b)
                        print(f"Saved grid channel coordinates (npy, B): {out_npy_b}")
                    if save_channels_csv:
                        base, ext = os.path.splitext(save_channels_csv)
                        out_csv_b = f"{base}_B{ext}"
                        os.makedirs(os.path.dirname(out_csv_b), exist_ok=True)
                        header = "index,row,col,x,y,z"
                        np.savetxt(out_csv_b, arr_b, delimiter=",", header=header, comments="")
                        print(f"Saved grid channel coordinates (csv, B): {out_csv_b}")

                # axis points along vb (distinct color)
                total_pts_b = int(num_channels)
                pts_list_b = []
                spacing_b = 1.0
                if total_pts_b % 2 == 1:
                    pts_list_b.append(p1_90_b)
                    k = 1
                    while len(pts_list_b) < total_pts_b:
                        pts_list_b.append(p1_90_b + k * spacing_b * vb)
                        if len(pts_list_b) >= total_pts_b:
                            break
                        pts_list_b.append(p1_90_b - k * spacing_b * vb)
                        k += 1
                else:
                    ...
                    while len(pts_list_b) < total_pts_b:
                        ...
                        k += 1
                if pts_list_b:
                    pts_b = np.vstack(pts_list_b)
                    plotter.add_mesh(pv.PolyData(pts_b), color="purple", render_points_as_spheres=True, point_size=channel_point_size)

                # axis points along wb = ub
                wb_dir = ub
                axis_wb_pts_all = []
                step_mm_b = 1.0
                max_k_wb = max(int(max(grid_rows, grid_cols)), 1)
                for k in range(1, max_k_wb + 1):
                    step = (k - 0.5) * step_mm_b
                    axis_wb_pts_all.append(p1_90_b + step * wb_dir)
                    axis_wb_pts_all.append(p1_90_b - step * wb_dir)
                if axis_wb_pts_all:
                    cap_b = min(len(axis_wb_pts_all), int(num_channels))
                    pts_wb = np.vstack(axis_wb_pts_all[:cap_b])
                    plotter.add_mesh(pv.PolyData(pts_wb), color="gold", render_points_as_spheres=True, point_size=channel_point_size)
...
def _parse_params_from_txt(txt_path: str, probe: str = "A") -> Tuple[float, float, float, float]:
    ...
    # 우선 프로브별 키를 찾고, 없으면 구형(프로브 구분 없는) 포맷으로 폴백
    p = (probe or "A").strip().upper()
    try:
        alpha = find_float(rf"Probe\s+{p}\s*-\s*Alpha:\s*([\d\.-]+)")
        beta = find_float(rf"Probe\s+{p}\s*-\s*Beta:\s*([\d\.-]+)")
        offset = find_float(rf"Probe\s+{p}\s*-\s*Offset\s*from\s*base:\s*([\d\.-]+)")
        shank = find_float(rf"Probe\s+{p}\s*-\s*Shank\s*length:\s*([\d\.-]+)")
...
def load_params_from_pkl(pkl_path: str, probe: str = "A") -> Tuple[float, float, float, float]:
    # Try reading parameters from sibling TXT first to avoid pickle issues
    txt_path = os.path.splitext(pkl_path)[0] + ".txt"
    if os.path.isfile(txt_path):
        try:
            return _parse_params_from_txt(txt_path, probe=probe)
        except Exception:
            pass
    ...
    # multi-probe dict e.g., alpha_A, alpha_B ...
    if isinstance(data, dict) and any(k.endswith("_A") or k.endswith("_B") for k in data.keys()):
        p = (probe or "A").strip().upper()
        suffix = f"_{p}"
        mapping = {
            "alpha": [f"alpha{suffix}", f"pitch{suffix}"],
            "beta": [f"beta{suffix}", f"yaw{suffix}"],
            "offset": [f"offset{suffix}", f"offset_mm{suffix}", f"offset_from_base{suffix}"],
            "shank_length": [f"shank_length{suffix}", f"shank_len{suffix}", f"shank_len_mm{suffix}"],
        }
        vals = {}
    ...
    if isinstance(data, (list, tuple)):
        for elem in data:
            if isinstance(elem, dict) and "x" in elem and hasattr(elem["x"], "__len__") and len(elem["x"]) >= 4:
                x = elem["x"]
                # 다중(두 전극) 최적화의 경우 8차원: [A 4개, B 4개]
                if len(x) >= 8:
                    if (probe or "A").strip().upper() == "A":
                        return float(x[0]), float(x[1]), float(x[2]), float(x[3])
                    else:
                        return float(x[4]), float(x[5]), float(x[6]), float(x[7])
                return float(x[0]), float(x[1]), float(x[2]), float(x[3])
...
def main():
    ...
    parser.add_argument("--pkl", type=str, default=r"C:\\Users\\user\\YongtaeC\\vimplant0812\\data\\1110_multi_electrode_100610\\100610_LH_V1_n150_5x10x10+5x10x10_dice-yield-HD_50_targ-full.pkl",
    ...
    parser.add_argument("--probe", type=str, choices=["A", "B", "both"], default="both",
                        help="멀티 전극 결과에서 사용할 프로브 선택 (기본: both)")
    parser.add_argument("--show_B_main", dest="show_B_main", action="store_true", default=False,
                        help="Probe B 메인(주황선/샹크/오프셋 구) 표시")
    parser.add_argument("--no_show_B_main", dest="show_B_main", action="store_false",
                        help="Probe B 메인 비표시(기본)")
    parser.add_argument("--show_B_channels", dest="show_B_channels", action="store_true", default=False,
                        help="Probe B 컨택트 파란 점 표시")
    parser.add_argument("--no_show_B_channels", dest="show_B_channels", action="store_false",
                        help="Probe B 컨택트 파란 점 비표시(기본)")
    ...
    if args.probe == "both":
        alpha, beta, offset_mm, shank_len_mm = load_params_from_pkl(args.pkl, probe="A")
        alpha_b, beta_b, offset_mm_b, shank_len_mm_b = load_params_from_pkl(args.pkl, probe="B")
    else:
        alpha, beta, offset_mm, shank_len_mm = load_params_from_pkl(args.pkl, probe=args.probe)
        alpha_b = beta_b = offset_mm_b = shank_len_mm_b = None
    ...
    visualize(surf_path,
              ...
              second_beta=beta_b,
              second_offset_mm=offset_mm_b,
              second_shank_len_mm=shank_len_mm_b,
              show_b_main=bool(getattr(args, "show_B_main", False)),
              show_b_channels=bool(getattr(args, "show_B_channels", False)),
              out_png=out_path,
