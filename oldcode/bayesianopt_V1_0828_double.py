...
try:
    ...
    _HAS_SKOPT_PLOTS = True
except Exception:
    _HAS_SKOPT_PLOTS = False

########################
### Custom functions ###
########################
from ninimplant import get_xyz # matrix rotation/translation ect
...
outputfolder = "C:/Users/user/YongtaeC/vimplant0812/data/output/102311_double/"
os.makedirs(outputfolder, exist_ok=True)

# determine range of parameters used in optimization (Probe A & B 각각 4개씩 = 8개)
dimA1 = Integer(name='alpha_A', low=-90, high=90) # visual degrees 
...
dimensions = [dimA1, dimA2, dimA3, dimA4, dimB1, dimB2, dimB3, dimB4]

num_calls = 150
x0 = (0,0,20,25, 0,0,20,25) # initial values for the eight dimensions
...
# --- Grid 커스텀 파라미터 (0819_7x7.ipynb 스타일) ---
# 원하는 그리드 크기/간격을 지정하세요. (예시 기본값)
grid_nx = 10  # x축 샹크 개수
grid_ny = 9   # y축 comb 개수
grid_nz = 8   # z축 컨택트 개수(샹크 방향)

# x, y 간격(mm). 기존 하나의 spacing을 쓰던 방식과 호환되도록 기본값은 spacing_along_xy와 동일하게 둡니다.
spacing_x = spacing_along_xy
spacing_y = spacing_along_xy

# --- Probe B(두 번째 전극) 설정 ---
grid2_nx = 7
grid2_ny = 6
grid2_nz = 5
spacing2_x = spacing_along_xy
spacing2_y = spacing_along_xy

# 두 전극 시작 위치 분리(기본: y로 약간 이동)
start_location_B_offset = [0.0, 5.0, 0.0]

# 최소 거리 제약(mm): 전극 간 최소거리는 spacing_along_xy 이상
min_interprobe_distance_mm = spacing_along_xy
...
def f_manual(alpha_A, beta_A, offset_from_base_A, shank_length_A,
    ...
    # create grid A (customizable x/y/z & spacing)
    ...
    # create grid B (두 번째 전극): 시작점 약간 이동
    start_location_B = [
    ...
    # implanting grids
    ref_contacts_xyz_A, contacts_xyz_moved_A, refline_A, refline_moved_A, projection_A, ref_orig_A, ray_visualize_A, new_location_A, grid_valid_A = implant_grid(gm_mask, orig_grid_A, start_location, new_angle_A, offset_from_base_A)
    ref_contacts_xyz_B, contacts_xyz_moved_B, refline_B, refline_moved_B, projection_B, ref_orig_B, ray_visualize_B, new_location_B, grid_valid_B = implant_grid(gm_mask, orig_grid_B, start_location_B, new_angle_B, offset_from_base_B)

    # merge contacts for phosphene generation
    ...
    # collision penalty and validity update
    distAB = _min_interprobe_distance_mm(contacts_xyz_moved_A, contacts_xyz_moved_B)
    collide_penalty = 0.0 if distAB >= min_interprobe_distance_mm else (min_interprobe_distance_mm - distAB) * 1e3
    cost = cost + collide_penalty
    
    # check if cost contains invalid value
    if np.isnan(cost) or np.isinf(cost):
        cost = 3
    
    grid_valid = bool(grid_valid_A and grid_valid_B and (distAB >= min_interprobe_distance_mm))
...
def main():
    ...
    # set beta angle constraints according to hemisphere (A,B 각각 설정)
    ...
    for target_density, ftarget in zip(targ_comb, targ_names):
        for (a, b, c), floss in zip(loss_comb, loss_names):
            ...
            for s in subj_list:
                ...
                for gm_mask, hem, start_location, good_coords, good_coords_V1, good_coords_V2, good_coords_V3, dim2_A, dim2_B in zip([GM_LH, GM_RH], ['LH', 'RH'], [median_lh, median_rh], [good_coords_lh, good_coords_rh], [V1_coords_lh, V1_coords_rh], [V2_coords_lh, V2_coords_rh], [V3_coords_lh, V3_coords_rh], [dim2_lh_A, dim2_rh_A], [dim2_lh_B, dim2_rh_B]):        
                    ...
                    if os.path.exists(fname):
                        ...
                        def f(alpha_A, beta_A, offset_from_base_A, shank_length_A, alpha_B, beta_B, offset_from_base_B, shank_length_B):
                            """
                            ...
                            * Ultimately, the function returns the calculated cost used by the bayesopt algorithm.
                            ...
                            # 전극 B는 동일 각도 사용(필요 시 다른 각도로 확장 가능)
                            ...
                            print('    ', "{:.2f}".format(cost), "{:.2f}".format(dice), "{:.2f}".format(grid_yield), "{:.2f}".format(par3), (grid_valid_A and grid_valid_B), 'dAB=', "{:.2f}".format(distAB))
                            return cost
