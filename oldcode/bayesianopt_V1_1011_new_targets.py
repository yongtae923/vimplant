...
# 현재 파일의 절대 경로를 기준으로 new_targets 폴더 경로 계산
current_dir = os.path.dirname(os.path.abspath(__file__))  # code 폴더
project_root = os.path.dirname(current_dir)  # 프로젝트 루트
new_targets_path = os.path.join(project_root, 'new_targets')

# new_targets 폴더를 Python path에 추가
if new_targets_path not in sys.path:
    sys.path.insert(0, new_targets_path)

try:
    from image_generator import ImageTargetGenerator
except ImportError as e:
    print(f"ImageTargetGenerator import failed: {e}")
    print(f"Trying to add path: {new_targets_path}")
    print(f"Path exists: {os.path.exists(new_targets_path)}")
    ImageTargetGenerator = None
...
# 서브젝트 ID 설정 (subj_list에서 첫 번째 서브젝트 사용)
subj_id = 100610  # 현재 설정된 서브젝트
datafolder = f"C:/Users/user/YongtaeC/vimplant0812/data/input/{subj_id}/"
outputfolder = f"C:/Users/user/YongtaeC/vimplant0812/data/output/{subj_id}_newtarget_1013/"
...
# 이미지 타겟 생성기 초기화
if ImageTargetGenerator is not None:
    image_generator = ImageTargetGenerator(output_dir="new_targets/processed_targets")
    
    # 이미지 타겟 맵들 로드 (없으면 생성)
    image_targets = []
    image_target_names = []
    
    # 알파벳 A, B, C 테스트 (기존에 생성된 것 우선 사용)
    letters = ['A', 'B', 'C']
    for letter in letters:
        target_name = f"letter_{letter}"
        target_map = image_generator.load_target_map(target_name)
        
        if target_map is None:
            print(f"기존 타겟 맵이 없어서 새로 생성 중: {letter}")
            target_map = image_generator.generate_from_text(letter, target_name, 
                                                           size=WINDOWSIZE, font_size=700, sigma=30)
        else:
            print(f"기존 타겟 맵 사용: {letter}")
        
        if target_map is not None:
            image_targets.append(target_map)
            image_target_names.append(f"targ-{letter.lower()}")
    
    # 겹친 알파벳 타겟 맵 생성 (A~C, A~Z) - 기존에 생성된 것 우선 사용
    overlapping_targets = [
        ("ABC", "letters_abc"),
        ("ABCDEFGHIJKLMNOPQRSTUVWXYZ", "letters_all")  # 새로 만든 파일명과 일치
    ]
    
    for letters_str, target_name in overlapping_targets:
        # 기존 타겟맵 로드 시도
        target_map = image_generator.load_target_map(target_name)
        
        if target_map is None:
            print(f"기존 타겟 맵이 없어서 새로 생성 중: {letters_str}")
            target_map = image_generator.generate_overlapping_alphabet(
                letters_str, target_name, font_size=700, sigma=30
            )
        else:
            print(f"기존 타겟 맵 사용: {letters_str}")
            
        if target_map is not None:
            image_targets.append(target_map)
            image_target_names.append(f"targ-{target_name}")
else:
    print("ImageTargetGenerator를 사용할 수 없습니다. 기본 타겟 맵만 사용합니다.")
    image_targets = []
    image_target_names = []

# 기존 타겟 구성 대신 processed_targets 폴더의 모든 NPY를 사용 (LH 기준)
processed_dir = os.path.join(project_root, 'new_targets', 'processed_targets')
loaded_targets = []
loaded_names = []
if os.path.isdir(processed_dir):
    for fname in sorted(os.listdir(processed_dir)):
        if fname.lower().endswith('.npy'):
            npy_path = os.path.join(processed_dir, fname)
            try:
                arr = np.load(npy_path)
                if isinstance(arr, np.ndarray) and arr.ndim == 2:
                    loaded_targets.append(arr.astype(np.float64))
                    stem = os.path.splitext(fname)[0]
                    loaded_names.append(f"targ-{stem}")
                    print(f"Loaded target: {fname}")
            except Exception as e:
                print(f"Failed to load {npy_path}: {e}\n")
else:
    print(f"Processed targets directory not found: {processed_dir}")

# 폴백: 비어있으면 기존 기본 타겟 + 이미지 타겟 사용
if len(loaded_targets) == 0:
    targ_comb = ([gvs.upper_sector(windowsize=WINDOWSIZE, fwhm=800, radiusLow=0, radiusHigh=500, plotting=False), 
                 ...
                 gvs.triangle_target(windowsize=WINDOWSIZE, fwhm=400, triangle_size=300, center=None, plotting=False)] + image_targets)
    targ_names = (['targ-upper', 'targ-lower', 'targ-inner', 'targ-full', 'targ-square', 'targ-triangle'] + image_target_names)
else:
    targ_comb = loaded_targets
    targ_names = loaded_names
...
subj_list = [subj_id]  # 위에서 설정한 서브젝트 ID 사용
    ...
    for target_density, ftarget in zip(targ_comb, targ_names):
        for (a, b, c), floss in zip(loss_comb, loss_names):
            ...
            for s in subj_list:
                ...
                # apply optimization to each hemisphere (LH and RH)
