...
- Start with all contacts active.
- At each step, test every active contact by removing exactly one contact.
- Remove the contact that gives the minimum trial loss (argmin).
- Stop when further removal does not improve the loss enough.
- Keep at least `min_active_before_stop` contacts.
"""
...
INPUT_NPZ_DIR = PROJECT_ROOT / "data" / "output" / "opt_npz_letter"
OUTPUT_NPZ_DIR = PROJECT_ROOT / "data" / "output" / "opt_select_npz_letter_v2"
OUTPUT_PLOT_DIR = PROJECT_ROOT / "data" / "output" / "opt_select_plot_letter_v2"
...
def greedy_select(
    ...
    while len(active) > min_active_to_keep:
        ...
        best_trial_active: list[int] | None = None
        best_trial_map: np.ndarray | None = None
        ...
        for i in active:
            ...
            if (best_trial_loss is None) or (trial_loss < best_trial_loss):
                ...
                best_i = i
                best_trial_active = trial_active
                best_trial_map = trial_map
                best_comps = comps

        if (
            best_i is None
            or best_trial_active is None
            or best_trial_map is None
            or best_trial_loss is None
            or best_comps is None
        ):
            break
...
def main() -> None:
    ...
    print("greedy strategy: start full, remove one contact with minimum trial loss")
